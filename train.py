...
def train_fn(disc_H, disc_Z, gen_Z, gen_H, loader, opt_disc, opt_gen, l1, mse, d_scaler, g_scaler):
    ...
    for idx, (zebra, horse) in enumerate(loop):
        ...
        with torch.cuda.amp.autocast():  # pytorch 使用amp.autocast半精度加速训练
            ...
            D_H_real_loss = mse(D_H_real, torch.ones_like(D_H_real))  # 论文中改进loss，训练更稳定
        ...
        if idx % 200 == 0:
            save_image(zebra * 0.5 + 0.5, f"saved_images/tzebra_{idx}.png")
            save_image(horse * 0.5 + 0.5, f"saved_images/thorse_{idx}.png")
            save_image(fake_horse * 0.5 + 0.5, f"saved_images/ghorse_{idx}.png")
            save_image(fake_zebra * 0.5 + 0.5, f"saved_images/gzebra_{idx}.png")
